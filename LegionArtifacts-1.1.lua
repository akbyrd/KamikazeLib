...
local function RefreshPowers()
   print("Refreshing powers")
   if C_ArtifactUI.GetPowers() then
      local itemID, altID, name,_,_,numPowers = C_ArtifactUI:GetArtifactInfo()
      print (itemID.."; "..name.."; "..numPowers)
      if not artifact[itemID] then artifact[itemID] = {} end
      count[itemID] = numPowers
      
      for i,p in ipairs(C_ArtifactUI.GetPowers()) do
         local powerInfo = C_ArtifactUI.GetPowerInfo(p)
         local powerName = GetSpellInfo(powerInfo.spellID)
         print (powerInfo.spellID.."; "..powerName.."; "..powerInfo.currentRank)
         artifact[itemID][powerInfo.spellID] = powerInfo
      end
   else
      --do nothing
   end
...
-- data updates
print("LegionArifacts Loaded")
local f = CreateFrame('frame')
f:SetScript('OnEvent', function(self, event, ...)
   if event == 'PLAYER_ENTERING_WORLD' then
      print("LegionArifacts Refreshing")
      RefreshEquipped()
      RefreshBags()
