...
function IconTest.InkRect(glyph, fontSize, flags)
...
function IconTest.FontSize(buttonSize, fraction)
...
function IconTest.PlaceIcon(fontString, glyph, fontSize, width, height)
	local flags   = glyph.flags or FLAGS
	local rect    = IconTest.InkRect(glyph, fontSize, flags)
...
local function CreateButton(parent, glyph, size, x, y)
	...
	button.size     = size
	button.glyph    = glyph
	button.fontSize = IconTest.FontSize(size, fraction)
	button.rect, button.xOffset, button.yOffset = IconTest.PlaceIcon(text, glyph, button.fontSize, size, size)
...
function IconTest.Load()
	...
	local Root = CreateFrame("Frame", "KL_ICON_TEST", UIParent)
	...
	for iGlyph, glyph in ipairs(glyphs) do
		...
		for iSize, size in ipairs(sizes) do
			...
			table.insert(IconTest.buttons, CreateButton(Root, glyph, size, x, y))
			x = x + size + pad
		end
	end

	IconTest.Root   = Root
	IconTest.guides = true
	IconTest.RefreshScale()
end

function IconTest.RefreshScale()
	local pixelsToUI = PixelUtil.GetPixelToUIUnitFactor() / UIParent:GetEffectiveScale()
	local pxWidth, pxHeight = GetPhysicalScreenSize()
	local Root = IconTest.Root
	Root:SetScale(pixelsToUI)
	Root:ClearAllPoints()
	Root:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT",
		floor((pxWidth - (IconTest.groupWidth or Root:GetWidth())) / 2),
		floor((pxHeight - Root:GetHeight()) / 2))
end

function IconTest.Toggle()
	IconTest.Root:SetShown(not IconTest.Root:IsShown())
end

function IconTest.ToggleGuides()
	IconTest.guides = not IconTest.guides
	for _, button in ipairs(IconTest.buttons) do
		button.GuideX:SetShown(IconTest.guides)
		button.GuideY:SetShown(IconTest.guides)
	end
end

-- Predicted/measured, in pixels
function IconTest.Dump()
	for _, button in ipairs(IconTest.buttons) do
		if button.glyph == glyphs[1] then
			local text = button.Text
			print(format("IconTest %s %dpx: font %d, offset %d,%d, width %d/%.2f, height %d/%.2f, font height %.2f, line height %.2f",
